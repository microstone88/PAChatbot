...
//						+ " AND `locale` = \"" + locale_lang + "\"";
	 ...
	 * @return -1, if failed; the row count, if succeeded.
	 */
	static int updateUserStatus(long uid, String ip, UStatus status) {
		String sql = "UPDATE `" + Tables.CLOGIN + "` "
	 ...
	 * @return -1, if failed; the row count, if succeeded.
	 */
	static int updateOnlyUserStatus(long uid, UStatus status) {
		String sql = "UPDATE `" + Tables.CLOGIN + "` "
	 ...
	 */
	static QueryResult updateClientInfo(long uid, TInfo.Column column, String value) {
